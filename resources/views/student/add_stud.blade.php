			...
			<label for="st_id">University Roll No</label>
                   ...
                   <b> Semester:<b>
					<select name='semester' id='sem' onchange="showBranch();" class="browser-default" required>
						...
						@foreach($sem as $s)
						<option value='{{$s->semester}}'> <span class="text-capitalize" >{{ strtolower($s->semester)}} </span></option>
						@endforeach
					</select> <br>
                    ...
                    @if(session('usertype')=="ADMIN")
                            <b>BRANCH</b>
                                   <select name='branch' id='branch' onchange="showsection()" class="browser-default" required></select>
                        @else
                                <input type="hidden" id="branch" name="branch" onload="showsection()" value="{{ session('branch') }}">
                           @endif

						<span class="text-capitalize">section</span>
					<select name='section' id='section' onfocus="showsection()" class="browser-default" required>
						
				</select>
			<br>
				<input type='submit' value='ADD STUDENT' name='add' class="btn green right-align form-control"  style="width:200px">
		</form>
	</div>
	@endsection